...
const device = fs
    .readFileSync(
        '/Library/Application Support/kicad/library/' + library_name + '.lib'
    )
    .toString()
    ...
    value(
        ...
        this.output += this.xmlTag`
            <text 
                ...
                transform="rotate(${orientation === 0 ? '0' : '-90'}, ${p.x}, ${
            p.y
        })" 
                x="${p.x}" y="${p.y}" ${text ? '' : 's:attribute="value"'}>
                    ${text}
            </text>
        `
    }
...
class SchSkinPlotter extends SchPlotter {
    ...
    plotDrawPinReference(draw, component, transform) {
        ...
        if (orientation === PinOrientation.UP) {
            ...
            props.position = 'top'
        } else if (orientation === PinOrientation.LEFT) {
...
const width = rect.width * scale + PADDING,
    height = rect.height * scale + PADDING
//
...
const x_mirror = 1
const y_mirror = 1
const x_scale = x_mirror * scale
const y_scale = y_mirror * scale
if (rotate == 0) transform = new Transform(x_scale, 0, 0, y_scale * -1) // Base
if (rotate == 90) transform = new Transform(0, scale, -1 * scale, 0) // 90
if (rotate == 180) transform = new Transform(-1 * scale, 0, 0, scale) // 180
if (rotate == 270 || rotate == -90)
    transform = new Transform(0, -1 * scale, 1 * scale, 0) // 270
// GND
pos = rect.pos1
...
svgPlotter.startG(
    { type, width: rect.width * scale, height: rect.height * scale },
    ''
)
